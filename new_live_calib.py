...
while capL.isOpened():
    ...
    if successL and successR:
        ...
        l_and_r = resize_images_and_combine(50, copL, copR)
        ...
        if k == 27:
            ...
            if retL and retR == True:
                ...
                last_calib_frame = resize_images_and_combine(50, imgL, imgR)
                #cv2.imshow('img right', l_and_r)
                cv2.waitKey(1000)

            elif retL == True:
                print('VALID LEFT CALIB')
                cornersL = cv2.cornerSubPix(grayL, cornersL, (11,11), (-1,-1), criteria)
                imgpointsL.append(cornersL)
                imgL, imgR = calibrate_rectify_undistort(objpoints, imgpointsL, imgpointsR, frameSize, grayL, grayR, imgL, imgR)
                last_calib_frame = resize_images_and_combine(50, imgL, imgR)
